...
namespace Challenges._1._GGStateMachineCharacterPhysics.Scripts.MonoBehaviours
{
    ...
    public class CharacterController : MonoBehaviour, IInputListener
    {
        ...
        private float _midAirVelocityDamping;
        private float _noInputVelocityDamping;
        private float _withInputVelocityDamping;
        ...
        /*private void FixedUpdate()
        {
            ...
            {
                _stateMachine.SwitchToState<MovingState, Vector3>(_moveDirection);
            }
        }*/
        public void SetConfigValues(CharacterMovementConfig parameters)
        {
            _acceleration = _moveDirection * parameters.AccelerationByTime * Time.deltaTime;
            _maxSpeed = parameters.MAXSpeed;
            _gravity = parameters.Gravity;
            _midAirVelocityDamping = parameters.MidAirXZVelocityDamping;
            _noInputVelocityDamping = parameters.NoInputVelocityDamping;
            _withInputVelocityDamping = parameters.WithInputVelocityDamping;
        }

        private void Movement()
        {
            _speed += characterMovementConfig.AccelerationByTime*5/6*Time.fixedDeltaTime;
            if(_speed > characterMovementConfig.MAXSpeed)
            {
                _speed = characterMovementConfig.MAXSpeed;
            }
        ...
        private void SetupStateMachineStates()
        {
            ...
            _stateMachine.RegisterUniqueState(new MovingState(characterMovementConfig.MAXSpeed, characterMovementConfig.AccelerationByTime, transform));
            _stateMachine.RegisterUniqueState(new FallingState(characterMovementConfig.Gravity, transform));
        }
        ...
        // (A and S) -> (-1,-1) ;
        // (A) -> (-1,0)
        public CharacterMovementConfig GetConfigValues()
        {
            return characterMovementConfig;
        }
        public void SetCurrentMovement(Vector2 xzPlaneMovementVector)
        {

            _moveDirection = new Vector3(xzPlaneMovementVector.x, 0, xzPlaneMovementVector.y);
            Movement();
        }
